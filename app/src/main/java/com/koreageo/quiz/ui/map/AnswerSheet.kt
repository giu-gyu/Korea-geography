...
fun AnswerSheet(
    ...
    // 화면 정중앙이 아니라 위/아래 기준 약 70% 지점(중앙보다 살짝 아래)에 띄운다 —
    // 정중앙에 뜨면 방금 탭한 작은 지역(서울특별시 등)이 다이얼로그에 가려져서
    // 어떤 지역이 눌렸는지 확인할 수가 없다.
    Dialog(onDismissRequest = onDismiss, properties = DialogProperties(usePlatformDefaultWidth = false)) {
        Box(modifier = Modifier.fillMaxSize(), contentAlignment = BiasAlignment(0f, 0.5f)) {
